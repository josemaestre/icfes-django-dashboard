...
// Initialize school search when DOM is ready
document.addEventListener('DOMContentLoaded', function() {
    const searchInput = document.getElementById('searchColegioInput');
    const searchBtn = document.getElementById('searchColegioBtn');
    
    if (searchInput) {
        searchInput.addEventListener('input', debounceSearch);
        searchBtn.addEventListener('click', performSchoolSearch);
    }
});
...
async function performSchoolSearch() {
    const query = document.getElementById('searchColegioInput').value;
    ...
    try {
        const response = await fetch(/icfes/api/search/colegios/?q=);
        const data = await response.json();
...
async function loadSchoolDetail(school) {
    ...
    document.getElementById('schoolDetailLocation').textContent = ${school.municipio}, ;
...
async function loadSchoolResumen(sk) {
    try {
        const response = await fetch(/icfes/api/colegio//resumen/);
        const data = await response.json();
...
async function loadSchoolHistorico(sk) {
    try {
        const response = await fetch(/icfes/api/colegio//historico/);
        const data = await response.json();
...
async function loadSchoolComparacion(sk) {
    try {
        const response = await fetch(/icfes/api/colegio//comparacion/);
        const data = await response.json();
